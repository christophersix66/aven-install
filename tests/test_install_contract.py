...
class InstallContractTests(unittest.TestCase):
    ...
    def test_check_mode_renders_exact_predecessor_and_target(self) -> None:
        ...
        with (
            ...
            mock.patch("installer.ensure_github_access"),
            mock.patch("installer.resolve_release"),
            mock.patch("installer.inspect_existing_aven", return_value="APPROVED_HEALTHY_PREDECESSOR:1.0.0-rc.18"),
            mock.patch("installer.download_release") as download,
            mock.patch("sys.stdout", stdout),
        ):
            result = installer.main(["--manifest", str(ROOT / "channels/rc.json"), "--channel", "rc", "--check"])
        self.assertEqual(result, 0)
        self.assertIn("Approved predecessor: 1.0.0-rc.18", stdout.getvalue())
        self.assertIn("Predecessor classification: APPROVED_HEALTHY_PREDECESSOR", stdout.getvalue())
        self.assertIn("Target release: 1.0.0-rc.19", stdout.getvalue())
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                ...
                self.assertEqual(
                    ...
                    "APPROVED_HEALTHY_PREDECESSOR:1.0.0-rc.18",
                )
    ...
    def test_exact_predecessor_upgrade_uses_owned_uninstall_then_target_setup(self) -> None:
        ...
        self.assertEqual(
            calls,
            ["uninstall:1.0.0-rc.18:--plan", "uninstall:1.0.0-rc.18:--apply", "setup:--plan", "setup:--apply"],
        )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                ...
                installer._invoke_predecessor_uninstall(
                    "linux", "--plan", predecessor_version="1.0.0-rc.18", inherit=False
                )
