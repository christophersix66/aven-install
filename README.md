...
The current default channel is explicitly `rc`: Aven `1.0.0-rc.16`. Stable `1.0.0` does not exist and is not silently substituted.
...
When run from a checkout, the entrypoint automatically uses the adjacent coordinator and channel files and verifies their compiled SHA-256 values. For a reproducible review, check out the exact installer commit accepted by your organization before inspection. The coordinator also fails closed unless the channel is the exact approved private Aven `v1.0.0-rc.16` identity and its tag, source commit, release posture, asset identity, and SHA-256 all match. Exact `1.0.0-rc.15` is the sole approved predecessor for this transition; older or failed candidates are rejected rather than accepted by version ordering.
