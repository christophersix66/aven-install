...
The current default channel is explicitly `rc`: Aven `1.0.0-rc.18`. Stable `1.0.0` does not exist and is not silently substituted.
...
When run from a checkout, the entrypoint automatically uses the adjacent coordinator and channel files and verifies their compiled SHA-256 values. For a reproducible review, check out the exact installer commit accepted by your organization before inspection. The coordinator also fails closed unless the channel is the exact approved private Aven `v1.0.0-rc.18` identity and its tag, source commit, release posture, asset identity, and SHA-256 all match. Exact healthy `1.0.0-rc.16` and exact immutable published-defective `1.0.0-rc.17` are the only approved predecessors. RC.16 uses normal health checks. RC.17 uses a narrowly bounded recovery classification that independently verifies its exact version, lock, bootstrap distribution, installation manifest, runtime, and component pins while bypassing only RC.17's known-broken high-level `status` check. Modified RC.17 installations, generic unhealthy installations, older releases, and failed candidates are rejected.
